...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if rf.currentRole == Leader {
		Debug(rf.debugStartTime, dStart, rf.me, "Starting agreement on new entry")
...
func (rf *Raft) PerformVoteRequest(targetPeerIndex int, args *RequestVoteArgs) {
	...
	if reply.Term == rf.currentTerm && reply.VoteGranted {
		...
		if len(rf.votesReceived) >= int(math.Ceil(float64(len(rf.peers)+1)/2)) {
			Debug(rf.debugStartTime, dVote, rf.me, "NODE HAS BECOME LEADER!")
...
func (rf *Raft) replicateLog(leaderPeerIndex int, followerPeerIndex int) {
	...
	Debug(rf.debugStartTime, dAppendEntries, rf.me, "Sending AppendEntriesRPC to node %d", followerPeerIndex)
	rf.mu.Unlock()
	...
	if receivedReply {
		...
		if rf.currentTerm == reply.Term && rf.currentRole == Leader && len(args.Entries) > 0 {
			Debug(rf.debugStartTime, dAppendEntries, rf.me, "Received reply to AppendEntriesRPC is %+v", reply)
			if reply.Success {
				...
				Debug(rf.debugStartTime, dAppendEntries, rf.me, "Successful AppendEntriesRPC to node %d, nextIndex of log is %d", followerPeerIndex, rf.nextIndex[followerPeerIndex])
				...
				if lastLogIndex > rf.commitIndex {
					...
					if N != -1 && N > rf.commitIndex {
						if rf.log[N].Term == rf.currentTerm {
							Debug(rf.debugStartTime, dApplyLogEntries, rf.me, "Leader: Signalling on CondVar to apply log entries, old commitIndex = %d, new commitIndex = %d, lastApplied = %d", rf.commitIndex, N, rf.lastApplied)
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	rf.mu.Lock()
	Debug(rf.debugStartTime, dAppendEntries, rf.me, "Received AppendEntriesRPC from node = %d", args.LeaderId)
	...
	if args.Term > rf.currentTerm {
		...
		Debug(rf.debugStartTime, dAppendEntries, rf.me, "Received AppendEntriesRPC with older term from node = %d with term = %d, rejecting with latest term %d", args.LeaderId, args.Term, rf.currentTerm)
		reply.Term = rf.currentTerm
		rf.receivedHeartbeatOrVoteGrant = false
	...
	if len(args.Entries) > 0 {
		...
		if logIndex >= args.PrevLogIndex || args.PrevLogIndex == 0 {
			...
			if args.PrevLogIndex == 0 {
				if logIndex != 0 {
					Debug(rf.debugStartTime, dAppendEntries, rf.me, "Expected log length is 0, Node has index(length) = %d, Truncating to 0", logIndex)
					rf.log = rf.log[:0]
				}
				...
				reply.Success = true
				Debug(rf.debugStartTime, dAppendEntries, rf.me, "Prev Log Length = 0,  Entries have been appended, New length = %d", len(rf.log)-1)

				if args.LeaderCommit > rf.commitIndex {
					lastLogIndex := len(rf.log) - 1
					rf.commitIndex = int(math.Min(float64(args.LeaderCommit), float64(lastLogIndex)))
					Debug(rf.debugStartTime, dApplyLogEntries, rf.me, "Follower: Signalling on CondVar to apply log entries, commitIndex = %d, lastApplied = %d", rf.commitIndex, rf.lastApplied)
					rf.applyEntriesCondVar.Signal()
				}
				...
				if logIndex > args.PrevLogIndex {
					Debug(rf.debugStartTime, dAppendEntries, rf.me, "Node has a greater log index = %d, truncating to expected log index = %d", logIndex, args.PrevLogIndex)
				...
				if rf.log[args.PrevLogIndex].Term == args.PrevLogTerm {
					...
					reply.Success = true
					Debug(rf.debugStartTime, dAppendEntries, rf.me, "Prev Log Length = %d,  Entries have been appended, New length = %d", args.PrevLogIndex, len(rf.log)-1)
					...
					Debug(rf.debugStartTime, dAppendEntries, rf.me, "Expected log term at log index %d is %d, Node has term = %d, deleting entry from node log", args.PrevLogIndex, args.PrevLogTerm, rf.log[args.PrevLogIndex].Term)
...
func (rf *Raft) applyLogEntries() {
	...
	for !rf.killed() {
		...
		Debug(rf.debugStartTime, dApplyLogEntries, rf.me, "Received Signal on CondVar to apply log entries")

		if rf.commitIndex > rf.lastApplied {
			Debug(rf.debugStartTime, dApplyLogEntries, rf.me, "Starting to apply log entries, commitIndex = %d, lastApplied = %d", rf.commitIndex, rf.lastApplied)
			for i := rf.lastApplied + 1; i <= rf.commitIndex; i++ {
				...
				*rf.applyChChan <- applyMsg
				rf.lastApplied = i
				Debug(rf.debugStartTime, dApplyLogEntries, rf.me, "Applied log entry, commitIndex = %d, lastApplied = %d", rf.commitIndex, rf.lastApplied)
			}
